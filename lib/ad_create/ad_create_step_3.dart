...
class _AdCreateStep3FormState extends State<AdCreateStep3Form> {
  ...
  Future<void> _pickManualStartDateTime() async {
    ...
    final DateTime initialBase = now.add(const Duration(minutes: 5));
    final DateTime minAllowed = now.add(const Duration(minutes: 3));

    final DateTime base = _manualStartDateTime ?? initialBase;
    ...
    if (selected.isBefore(minAllowed)) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text("시작 일자/시간 설정은 현재 시각 기준 3분 뒤부터 설정할 수 있어요."),
        ),
      );
                          ...
                          return Expanded(
                            child: GestureDetector(
                              onTap: () {
                                ...
                                context.read<AdCreationProvider>().setAdMethod(method);
                              },
                              child: AnimatedContainer(
              ...
              child: ElevatedButton(
                onPressed: () {
                  ...
                  if(cleanText.length > 7){
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(
                          content: Text("예산은 최대 7자리까지만 입력 할 수 있습니다."),
                      ),
                    );
                    return;
                  }
