...
class _MainBoardFormState extends State<MainBoardForm> {
  ...
  /// 1: 준비중/대기중/중단중
  /// 2: 완료됨
  int _statusGroup(dynamic ad) {
    if (_isFinished(ad)) return 2;

    final raw = (ad['adstate'] as String?) ?? '';
    final started = _hasStarted(ad);

    if (raw == 'ACTIVE' && started) return 0;
  ...
  void _sortAds() {
    _myAds.sort((a, b) {
      final int ga = _statusGroup(a);
      final int gb = _statusGroup(b);

      // 1) 상태 그룹: 진행중(0) → 준비중/중단중(1) → 완료됨(2)
      if (ga != gb) {
  ...
  void _calculateStats() {
    ...
    for (var ad in _myAds) {
      ...
      if (rawState == 'ACTIVE' && remaining > 0 && _hasStarted(ad)) {
        ongoing++;
      }
    }
  ...
  bool _hasStarted(dynamic ad) {
    final raw = ad['start_date']?.toString();
  ...
  Color _statusColorFromAd(dynamic ad) {
    ...
    final hasStarted = _hasStarted(ad);
    if (!hasStarted) {
